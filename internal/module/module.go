...
func New(osPath string) (*Module, error) {
	...
	if modRoot == "" {
		return nil, fmt.Errorf("module file %s not found", moduleFileName)
	}
	absModRoot, err := filepath.Abs(modRoot)
	if err != nil {
		return nil, fmt.Errorf("invalid path %q: %v", modRoot, modRoot)
	}
	info := Module{
		root: absModRoot,
	...
	if err != nil {
		return nil, fmt.Errorf("cannot read %s: %v", absModRoot, err)
	}
	info.mod, err = modfile.Parse(modPath, modData, nil)
	if err != nil {
		return nil, fmt.Errorf("cannot parse %s: %v", absModRoot, err)
	}
...
// Path returns the parent directory of go.mod.
